...
pub struct WebRtc {
  ...
  tx: futures_util::lock::Mutex<tokio::sync::mpsc::Sender<Message>>,
  rx: Arc<futures_util::lock::Mutex<tokio::sync::mpsc::Receiver<Message>>>,
}
#[napi]
impl WebRtc {
  #[napi(factory)]
  pub fn create(conf: Configuration) -> napi::Result<Self> {
    let (tx, rx) = tokio::sync::mpsc::channel(100);
    let web = WebRtc {
      tx: futures_util::lock::Mutex::new(tx),
      rx: Arc::new(futures_util::lock::Mutex::new(rx)),
      config: conf,
      data_callback: None,
      send_callback: None,
      close_callback: None,
  ...
  #[napi]
  pub async fn start_capture() -> External<Arc<Mutex<ScreenDuplicator>>> {
    ...
    */
    ...
    External::new(Arc::new(Mutex::new(dupl)))
  }

  #[napi]
  pub fn init(&self, capture_loop: External<Arc<Mutex<ScreenDuplicator>>>) {
    ...
    let dupl = capture_loop.lock();
    let desc = dupl.desc().clone();
    let device = dupl.d3d11_device.clone();
    let context = dupl.d3d_ctx.clone();
    drop(dupl);
    //release the lock
    //drop(dupl);
