  ...
  fn build(
    ...
    let (codec, profile) = {
      match codec_capability.mime_type.as_str() {
        "video/H264" => {
          let profile = match h264_profile_from_sdp_fmtp_line(&codec_capability.sdp_fmtp_line) {
    ...
    let new_settings = supported_encode_presets.contains(&nvenc::EncodePreset::P3);
    ...
    let (preset, tuning_info, multi_pass, _rc_mode) = if new_settings {
      ...
      (
        nvenc::EncodePreset::P2,
        nvenc::TuningInfo::LowLatency,
    ...
    let (width, height, texture_format) = {
      //let display_desc: windows::Win32::Graphics::Dxgi::DXGI_OUTDUPL_DESC = screen_duplicator.desc();
      let mode_desc = self.dupl_desc.ModeDesc;
...
impl NvidiaEncoderBuilder {
  pub fn new(
    ...
    unsafe {
      ...
      println!("device flags: {:?}", device.GetCreationFlags());
    }
